...
from pyreachability import Graph, TFLabel, catalog
...
def test_cross_level_edges():
    # 0->1->2->3->4 chain plus long cross-level edges 0->4 and 0->3
    g = Graph.from_edges(np.array([0, 1, 2, 3, 0, 0], np.int32),
                         np.array([1, 2, 3, 4, 4, 3], np.int32), num_nodes=5)
    idx = TFLabel(); idx.build(g)
    for u in range(5):
        for v in range(5):
            assert idx.query(u, v) == (v >= u)
...
def test_errors_and_registration():
    ...
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "tfl" in catalog.methods()
    assert catalog.get("tfl") is TFLabel
...
@given(digraphs())
def test_tflabel_matches_oracle(g):
    n, edges = g
    src = np.array([e[0] for e in edges], dtype=np.int32)
    dst = np.array([e[1] for e in edges], dtype=np.int32)
    graph = Graph.from_edges(src, dst, num_nodes=n)
    idx = TFLabel(); idx.build(graph)
