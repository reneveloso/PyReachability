...
from pyreachability import Graph, PLL, catalog
...
def test_errors_and_registration():
    ...
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "pll" in catalog.methods()
    assert catalog.get("pll") is PLL
