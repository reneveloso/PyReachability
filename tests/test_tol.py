...
from pyreachability import Graph, TOL, catalog
...
def test_errors_and_registration():
    ...
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "tol" in catalog.methods()
    assert catalog.get("tol") is TOL
