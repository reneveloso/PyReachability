...
from pyreachability import Graph, PReaCH, catalog
...
def test_errors_and_registration():
    ...
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "preach" in catalog.methods()
    assert catalog.get("preach") is PReaCH
