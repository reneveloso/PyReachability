...
from pyreachability import Graph, Ferrari, catalog
...
def test_params_and_registration():
    ...
    idx = Ferrari(k=1); idx.build(g)
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "ferrari" in catalog.methods()
    assert catalog.get("ferrari") is Ferrari
...
def test_ferrari_matches_oracle(g, k):
    n, edges = g
    src = np.array([e[0] for e in edges], dtype=np.int32)
    dst = np.array([e[1] for e in edges], dtype=np.int32)
    graph = Graph.from_edges(src, dst, num_nodes=n)
    idx = Ferrari(k=k); idx.build(graph)
