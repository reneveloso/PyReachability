...
from pyreachability import Graph, TwoHop, catalog
...
def test_batch_matches_query():
    g = Graph.from_edges(np.array([0, 0, 1, 2], np.int32),
                         np.array([1, 2, 3, 3], np.int32), num_nodes=4)
    idx = TwoHop(); idx.build(g)
    pairs = np.array([[0, 3], [3, 0], [1, 1], [2, 3]], dtype=np.int32)
    out = idx.query_batch(pairs)
    assert out.tolist() == [idx.query(int(u), int(v)) for u, v in pairs]
...
def test_errors_and_registration():
    ...
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "twohop" in catalog.methods()
    assert catalog.get("twohop") is TwoHop
