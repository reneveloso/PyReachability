...
from pyreachability import Graph, DualLabeling, catalog
...
def test_errors_and_registration():
    with pytest.raises(RuntimeError):
        DualLabeling().query(0, 1)
    g = Graph.from_edges(np.array([0], np.int32), np.array([1], np.int32), num_nodes=2)
    idx = DualLabeling(); idx.build(g)
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "dual" in catalog.methods()
    assert catalog.get("dual") is DualLabeling
...
@given(digraphs())
def test_dual_matches_oracle(g):
    n, edges = g
    src = np.array([e[0] for e in edges], dtype=np.int32)
    dst = np.array([e[1] for e in edges], dtype=np.int32)
    graph = Graph.from_edges(src, dst, num_nodes=n)
    idx = DualLabeling(); idx.build(graph)
