...
from pyreachability import Graph, BFL, catalog
...
def test_errors_and_registration():
    ...
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "bfl" in catalog.methods()
    assert catalog.get("bfl") is BFL
...
@given(digraphs(), st.sampled_from([1, 3, 5]))
def test_bfl_matches_oracle(g, k):
    n, edges = g
    src = np.array([e[0] for e in edges], dtype=np.int32)
    dst = np.array([e[1] for e in edges], dtype=np.int32)
    graph = Graph.from_edges(src, dst, num_nodes=n)
    idx = BFL(k=k, seed=99); idx.build(graph)
