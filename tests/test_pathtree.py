...
from pyreachability import Graph, PathTree, catalog
...
def test_errors_and_registration():
    ...
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "pathtree" in catalog.methods()
    assert catalog.get("pathtree") is PathTree
...
@given(digraphs())
def test_pathtree_matches_oracle(g):
    n, edges = g
    src = np.array([e[0] for e in edges], dtype=np.int32)
    dst = np.array([e[1] for e in edges], dtype=np.int32)
    graph = Graph.from_edges(src, dst, num_nodes=n)
    idx = PathTree(); idx.build(graph)
