...
from pyreachability import Graph, DL, PLL, catalog
...
def test_errors_and_registration():
    ...
    with pytest.raises(IndexError):
        idx.query(0, 99)
    assert "dl" in catalog.methods()
    assert catalog.get("dl") is DL
...
@given(digraphs())
def test_dl_matches_oracle_and_pll(g):
    n, edges = g
    src = np.array([e[0] for e in edges], dtype=np.int32)
    dst = np.array([e[1] for e in edges], dtype=np.int32)
    graph = Graph.from_edges(src, dst, num_nodes=n)
    dl = DL(); dl.build(graph)
    pll = PLL(); pll.build(graph)
    reach = reachable_set(n, edges)
    for u in range(n):
        for v in range(n):
            ans = oracle_query(reach, u, v)
            assert dl.query(u, v) == ans
            assert dl.query(u, v) == pll.query(u, v)   # DL == PLL (survey-proven equivalent)
