...
cdef extern from "reachability/treesspi.hpp" namespace "reachability":
    cdef cppclass TreeSSPI:
        TreeSSPI() except +
        void build(const CSRGraph&) except +
        cbool reaches(vid_t, vid_t) nogil
        size_t index_size_bytes()

cdef extern from "reachability/gripp.hpp" namespace "reachability":
    cdef cppclass GRIPP:
        GRIPP() except +
        void build(const CSRGraph&) except +
        cbool reaches(vid_t, vid_t) nogil
...
cdef extern from "reachability/optchaincover.hpp" namespace "reachability":
    cdef cppclass OptimalChainCover:
        OptimalChainCover() except +
        void build(const CSRGraph&) except +
        cbool query(vid_t, vid_t) nogil
        size_t index_size_bytes()
