...
from ._core cimport CSRGraph, vid_t, bfs_reaches, Condensation, scc_condense, Grail, Feline, PLL, TC, TreeCover, BFL, ChainCover, PReaCH, TwoHop, TFLabel, TOL, HL, OReach, ThreeHop, PathHop, Ferrari, DualLabeling, TreeSSPI, GRIPP, PathTree, IP
...
cdef class _DualLabelingCore:
    cdef DualLabeling* _dl
    cdef vector[vid_t] _comp
    cdef bint _built

    def __cinit__(self):
        self._dl = new DualLabeling()
        self._built = False
        ...
        with nogil:
            for i in range(m):
                ...
                if cu == cv:
                    ...
                    ov[i] = 1 if dl.query(cu, cv) else 0
        return out.view(np.bool_)

    def index_size_bytes(self):
        return self._dl.index_size_bytes() + self._comp.size() * sizeof(int)

cdef class _TreeSSPICore:
    cdef TreeSSPI* _ts
    cdef vector[vid_t] _comp
    cdef bint _built

    def __cinit__(self):
        self._ts = new TreeSSPI()
        self._built = False
        ...
        with nogil:
            for i in range(m):
                ...
                if cu == cv:
                    ...
                    ov[i] = 1 if ts.reaches(cu, cv) else 0
        return out.view(np.bool_)

    def index_size_bytes(self):
        return self._ts.index_size_bytes() + self._comp.size() * sizeof(int)

cdef class _GRIPPCore:
    cdef GRIPP* _gp
    cdef vector[vid_t] _comp
    cdef bint _built
        ...
        with nogil:
            for i in range(m):
                ...
                if cu == cv:
                    ...
                    ov[i] = 1 if gp.reaches(cu, cv) else 0
        return out.view(np.bool_)

    def index_size_bytes(self):
        return self._gp.index_size_bytes() + self._comp.size() * sizeof(int)

cdef class _PathTreeCore:
    cdef PathTree* _pt
    cdef vector[vid_t] _comp
    cdef bint _built

    def __cinit__(self):
        self._pt = new PathTree()
        self._built = False
        ...
        with nogil:
            for i in range(m):
                ...
                if cu == cv:
                    ...
                    ov[i] = 1 if pt.query(cu, cv) else 0
        return out.view(np.bool_)

    def index_size_bytes(self):
        return self._pt.index_size_bytes() + self._comp.size() * sizeof(int)
        ...
        with nogil:
            for i in range(m):
                ...
                if cu == cv:
                    ...
                    ov[i] = 1 if ip.reaches(cu, cv) else 0
        return out.view(np.bool_)

    def index_size_bytes(self):
        return self._ip.index_size_bytes() + self._comp.size() * sizeof(int)
