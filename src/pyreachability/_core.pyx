...
cdef class _OptimalChainCoverCore:
    cdef OptimalChainCover* _oc
    cdef vector[vid_t] _comp
    cdef bint _built

    def __cinit__(self):
        self._oc = new OptimalChainCover()
        self._built = False
    ...
    def query_batch(self, pairs):
        ...
        cdef cnp.uint8_t[::1] ov = out
        cdef vector[vid_t]* comp = &self._comp
        cdef OptimalChainCover* oc = self._oc
        cdef vid_t cu, cv
        with nogil:
            for i in range(m):
                cu = comp[0][pv[i, 0]]; cv = comp[0][pv[i, 1]]
                if cu == cv:
                    ...
                    ov[i] = 1 if oc.query(cu, cv) else 0
        return out.view(np.bool_)
