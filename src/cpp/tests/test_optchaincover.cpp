...
TEST_CASE("optimal chain cover matches brute force on dags") {
    ...
    for (auto& gr : graphs) {
        ...
        OptimalChainCover idx; idx.build(g);
        for (vid_t u = 0; u < n; ++u)
            for (vid_t v = 0; v < n; ++v)
...
TEST_CASE("optimal chain cover handles empty and disconnected graphs") {
    CSRGraph empty(0, {}, {});
    OptimalChainCover e; e.build(empty);

    CSRGraph iso(4, {}, {});
    OptimalChainCover idx; idx.build(iso);
    for (vid_t u = 0; u < 4; ++u)
        for (vid_t v = 0; v < 4; ++v)
