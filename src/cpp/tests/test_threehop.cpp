...
TEST_CASE("3-hop matches brute force on dags") {
...
TEST_CASE("3-hop handles empty and disconnected graphs") {
    CSRGraph empty(0, {}, {});
    ThreeHop e; e.build(empty);

    CSRGraph iso(4, {}, {});
    ThreeHop idx; idx.build(iso);
    for (vid_t u = 0; u < 4; ++u)
        for (vid_t v = 0; v < 4; ++v)
