...
TEST_CASE("gripp matches brute force on dags") {
...
TEST_CASE("gripp handles empty and disconnected graphs") {
    CSRGraph empty(0, {}, {});
    GRIPP e; e.build(empty);

    CSRGraph iso(4, {}, {});
    GRIPP idx; idx.build(iso);
    for (vid_t u = 0; u < 4; ++u)
        for (vid_t v = 0; v < 4; ++v)
