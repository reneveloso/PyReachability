...
TEST_CASE("2-hop cover matches brute force on dags") {
...
TEST_CASE("2-hop handles empty and disconnected graphs") {
    CSRGraph empty(0, {}, {});
    TwoHop e; e.build(empty);

    CSRGraph iso(4, {}, {});            // 4 isolated vertices, no edges
    TwoHop idx; idx.build(iso);
    for (vid_t u = 0; u < 4; ++u)
        for (vid_t v = 0; v < 4; ++v)
