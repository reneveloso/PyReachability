...
TEST_CASE("path-tree matches brute force on dags") {
...
TEST_CASE("path-tree handles empty and disconnected graphs") {
    CSRGraph empty(0, {}, {});
    PathTree e; e.build(empty);

    CSRGraph iso(4, {}, {});
    PathTree idx; idx.build(iso);
    for (vid_t u = 0; u < 4; ++u)
        for (vid_t v = 0; v < 4; ++v)
