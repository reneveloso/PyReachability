...
TEST_CASE("TF-label matches brute force on dags") {
    std::vector<std::pair<std::vector<vid_t>, std::vector<vid_t>>> graphs = {
        {{0,0,1,2,2}, {1,2,3,3,4}},
        {{0,1,2,3,0}, {1,2,3,4,4}},          // has cross-level edge 0->4
        {{0,0,0,1,2,3}, {1,2,3,4,4,4}},      // many cross-level edges
        {{0,1,2,3}, {1,2,3,4}},              // single chain
        {{0,0,1,1,2,3}, {1,2,3,4,4,4}},
    };
    for (auto& gr : graphs) {
        vid_t n = 5;
        CSRGraph g(n, gr.first, gr.second);
        TFLabel idx; idx.build(g);
        for (vid_t u = 0; u < n; ++u)
            for (vid_t v = 0; v < n; ++v)
...
TEST_CASE("TF-label handles empty and disconnected graphs") {
    CSRGraph empty(0, {}, {});
    TFLabel e; e.build(empty);

    CSRGraph iso(4, {}, {});
    TFLabel idx; idx.build(iso);
    for (vid_t u = 0; u < 4; ++u)
        for (vid_t v = 0; v < 4; ++v)
