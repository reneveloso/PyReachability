...
namespace {
// DFS preorder phi and subtree-max phi_hat over a graph, starting from the given roots
// (iterative). phi is contiguous over each subtree, so phi_hat[v] = counter-1 at v's exit.
    ...
    for (vid_t r : roots) {
        ...
        while (!st.empty()) {
            ...
            if (st[top].it != st[top].end) {
                ...
                phi_hat[st[top].v] = counter - 1;   // contiguous preorder over the subtree
                st.pop_back();
            }
        }
    }
...
void PReaCH::build(const CSRGraph& dag) {
    ...
    dfs_ranges(dag, froots, pf_, pf_hat_);
    dfs_ranges(rdag, broots, pb_, pb_hat_);
}

bool PReaCH::reaches(vid_t u, vid_t v) {
    if (u == v) return true;

    // Constant-time cuts (Lemmas 2-4), forward and backward.
    if (pf_[u] <= pf_[v] && pf_[v] <= pf_hat_[u]) return true;     // forward DFS range (positive)
    if (lf_[u] >= lf_[v]) return false;                            // forward level (negative)
    if (pf_[v] > pf_hat_[u]) return false;                         // forward range (negative)
    if (pb_[v] <= pb_[u] && pb_[u] <= pb_hat_[v]) return true;     // backward DFS range (positive)
    if (lb_[v] >= lb_[u]) return false;                            // backward level (negative)
    if (pb_[u] > pb_hat_[v]) return false;                         // backward range (negative)
    ...
    while (fh < fq.size() || bh < bq.size()) {
        if (fh < fq.size()) {
            ...
            for (const vid_t* it = fwd_.out_begin(x); it != fwd_.out_end(x); ++it) {
                ...
                if (lf_[w] >= lft || pft > pf_hat_[w]) continue;   // w cannot reach v
                if (pf_[w] <= pft && pft <= pf_hat_[w]) return true;  // positive range
        ...
        if (bh < bq.size()) {
            ...
            for (const vid_t* it = bwd_.out_begin(y); it != bwd_.out_end(y); ++it) {
                ...
                if (lb_[z] >= lbs || pbs > pb_hat_[z]) continue;   // u cannot reach z
                if (pb_[z] <= pbs && pbs <= pb_hat_[z]) return true;
...
std::size_t PReaCH::index_size_bytes() const {
    return (lf_.size() + lb_.size() + pf_.size() + pf_hat_.size() +
            pb_.size() + pb_hat_.size()) * sizeof(vid_t) +
